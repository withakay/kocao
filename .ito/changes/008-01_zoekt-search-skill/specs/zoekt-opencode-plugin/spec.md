...
The OpenCode plugin SHALL trigger `agent-zoekt index` when files are edited, debouncing rapid successive edits to avoid excessive reindexing. The debounce interval SHALL be configurable with a sensible default (e.g., 30 seconds).
...
- **THEN** the plugin triggers `agent-zoekt index` if the index is older than the most recently modified tracked file
...
The plugin SHALL run `agent-zoekt index` in the background without blocking the agent's current operation. Index failures SHALL be logged but SHALL NOT interrupt agent workflows.
...
- **THEN** the `agent-zoekt index` process runs asynchronously and does not block the current agent tool call or user interaction

#### Scenario: Reindex failure is logged

- **WHEN** `agent-zoekt index` fails (e.g., binary not found, disk full)
