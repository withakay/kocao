...
Define an implementation-ready dashboard structure for Wave 3 that is centered on remote-agent pools, remote agents, task dispatch records, transcripts, and artifacts. The UI must not treat raw harness-run ids as the primary navigation model.
...
The dashboard is organized around four first-class resources.
