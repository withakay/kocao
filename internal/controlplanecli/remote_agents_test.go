...
func TestRemoteAgentTaskDispatchWithPromptFile(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/v1/remote-agent-tasks" || r.Method != http.MethodPost {
			http.NotFound(w, r)
			return
		}
		...
		if target["agentName"] != "reviewer" {
			t.Fatalf("agentName = %#v", target["agentName"])
		}
		if target["poolName"] != "backend" {
			t.Fatalf("poolName = %#v", target["poolName"])
		}
